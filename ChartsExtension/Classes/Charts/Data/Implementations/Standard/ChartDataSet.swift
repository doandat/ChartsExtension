...
open class ChartDataSet: ChartBaseDataSet
{
    ...
    open override func entriesForXValue(_ xValue: Double, _ yValue: Any?, _ x: Any?, _ y: Any?, _ valueToPixelMatrix: Any?) -> [ChartDataEntry]
    {
        var entries = [ChartDataEntry]()
        
//        var offetX = 0.0
//        var offetY = 0.0
//        if let x = x as? CGFloat, let yValue = yValue as? CGFloat, let y = y as? CGFloat{
//            let ratioX = xValue/Double(x)
//            let ratioY = Double(yValue)/Double(y)
////            offetX = 10*ratioX
////            offetY = 10*ratioY
//            offetX = 0.7
//            offetY = offetX*200/24
//
//        }
        var low = startIndex
        var high = endIndex - 1
            ...
            if let valueToPixelMatrix = valueToPixelMatrix as? CGAffineTransform {
                ...
                if let x = x as? CGFloat,let y = y as? CGFloat, pt.x - offset <= x && pt.x + offset >= x {
                    if barChart {
                        ...
                        if pt.y - offset <= y && pt.y + offset >= y {
                            haveAMatch = true
                        }
                    }
                }
            } else {
                haveAMatch = xValue == entry.x
            }

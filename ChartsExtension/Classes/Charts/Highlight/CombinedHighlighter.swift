...
open class CombinedHighlighter: ChartHighlighter
{
    ...
    open override func getHighlights(xValue: Double,yValue: Any?, x: CGFloat, y: CGFloat) -> [Highlight]
    {
        var vals = [Highlight]()
        ...
        for i in 0..<dataObjects.count
        {
            ...
            else
            {
                for j in 0..<dataObject.dataSetCount
                {
                    ...
                    let highs = buildHighlights(dataSet: dataSet, dataSetIndex: j, xValue: xValue,yValue: yValue, x: x, y: y, rounding: .closest)

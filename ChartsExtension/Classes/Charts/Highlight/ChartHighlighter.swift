...
open class ChartHighlighter : NSObject, IHighlighter
{
    ...
    {
        let val: CGPoint = getValsForTouch(x: x, y: y)
        let xVal = Double(val.x)
        let yVal = Double(val.y)
        return getHighlight(xValue: xVal, yValue: yVal, x: x, y: y)
    }
    ...
    @objc open func getHighlight(xValue xVal: Double, yValue yVal: Any?, x: CGFloat, y: CGFloat) -> Highlight?
    {
        guard let chart = chart else { return nil }
        
        let closestValues = getHighlights(xValue: xVal, yValue: yVal, x: x, y: y)
    ...
    @objc open func getHighlights(xValue: Double,yValue: Any?, x: CGFloat, y: CGFloat) -> [Highlight]
    {
        var vals = [Highlight]()
        ...
        for i in 0 ..< data.dataSetCount
        {
            ...
            vals.append(contentsOf: buildHighlights(dataSet: dataSet, dataSetIndex: i, xValue: xValue, yValue: yValue,x: x,y: y, rounding: .closest))
        }
...
//            entries = set.entriesForXValue(closest.x, closest.y)
//            entries = set.entriesForXValue(closest.x, closest.y,x,y)
        }

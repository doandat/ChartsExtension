...
{
    @objc open var circleRadius:CGFloat = CGFloat(3.0)
    
    @objc open var circleColor = NSUIColor.gray
    
    
    @objc open  func getCircleColor() -> NSUIColor {
        return circleColor
    }
    ...
    @objc open var drawCirclesEnabled = false
    @objc open var isDrawCirclesEnabled: Bool { return drawCirclesEnabled }
    @objc open var disableZeroValue = false
    
    @objc open var extendRightAreaEnabled = false
    @objc open var isExtendRightAreaEnabled: Bool { return extendRightAreaEnabled }

    @objc public init(viewPortHandler: ViewPortHandler, xAxis: XAxis?, transformer: Transformer?, circleRadius: CGFloat, circleColor: NSUIColor, drawCirclesEnabled: Bool, extendRightAreaEnabled: Bool, disableZeroValue: Bool)
    {
        super.init(viewPortHandler: viewPortHandler, transformer: transformer, axis: xAxis)
        self.circleRadius = circleRadius
        self.setCircleColor(circleColor)
        self.drawCirclesEnabled = drawCirclesEnabled
        self.extendRightAreaEnabled = extendRightAreaEnabled
        self.disableZeroValue = disableZeroValue
    }
    ...
    open override func renderAxisLabels(context: CGContext)
    {
        ...
        else if xAxis.labelPosition == .bottom
        {
            ...
            if isDrawCirclesEnabled {
                ...
                context.saveGState()
                let maxEntries = isExtendRightAreaEnabled ? entries.count - 1 : entries.count
                for i in stride(from: 0, to: maxEntries, by: 1)
                {
                    pt.x = CGFloat(entries[i])
                    pt.y = viewPortHandler.contentBottom
                    pt = pt.applying(valueToPixelMatrix)
                    rect.origin.x = pt.x - circleRadius
                    rect.origin.y = viewPortHandler.contentBottom - circleRadius
                    rect.size.width = circleRadius * 2

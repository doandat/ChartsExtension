...
{
    @objc open var arrValueYAxis = [ChartYValueCustom]()
    @objc public init(viewPortHandler: ViewPortHandler, yAxis: YAxis?, transformer: Transformer?, arrValueYAxis: [ChartYValueCustom])
    {
        super.init(viewPortHandler: viewPortHandler, transformer: transformer, axis: yAxis)
        self.arrValueYAxis = arrValueYAxis
    }
        ...
        textAlign: NSTextAlignment)
    {
        ...
        if arrValueYAxis.count > 0 {
            for i in stride(from: 0, to: arrValueYAxis.count, by: 1) {
                let valueY = arrValueYAxis[i]
                var isDrawLabel = !valueY.optional
                if valueY.optional {
                    let heightLabel = valueY.displayValue.size(withAttributes: [NSAttributedString.Key.font: labelFont, NSAttributedString.Key.foregroundColor: valueY.colorYValue]).height
                    for k in stride(from: 0, to: arrValueYAxis.count, by: 1) {
                        let valueYCheck = arrValueYAxis[k]
                        if !valueYCheck.optional {
                            let isConflictBottom = positions[i].y > positions[k].y && positions[i].y < positions[k].y + heightLabel
                            let isConflictTop = positions[i].y < positions[k].y && positions[i].y + heightLabel > positions[k].y
                            isDrawLabel = !(isConflictBottom || isConflictTop)
                            if !isDrawLabel {
                                break;
                            }
                        }
                ...
                if isDrawLabel {
                    ChartUtils.drawText(
                        context: context,
                        text: "\(valueY.displayValue)",
                        point: CGPoint(x: fixedPosition, y: positions[i].y + offset),
                        align: textAlign,
                        attributes: [NSAttributedString.Key.font: labelFont, NSAttributedString.Key.foregroundColor: valueY.colorYValue])
                }
...
//            context.setStrokeColor(yAxis.gridColor.cgColor)
            ...
            if arrValueYAxis.count > 0 {
                for k in 0..<arrValueYAxis.count {
                    let chartYValueCustom = arrValueYAxis[k]
                    if chartYValueCustom.value != 0.0 {
                        context.setStrokeColor(chartYValueCustom.colorLine.cgColor)
                        if chartYValueCustom.lineDashLengths.count > 0 {
                            context.setLineDash(phase: yAxis.gridLineDashPhase, lengths: chartYValueCustom.lineDashLengths)
                        } else {
                            context.setLineDash(phase: 0.0, lengths: [])
                        }
                        drawGridLine(context: context, position: positions[k])
                    }
//                    drawGridLine(context: context, position: positions)
                }
                // draw the grid
...
//            positions.forEach { drawGridLine(context: context, position: $0) }
    ...
    @objc open func transformedPositions() -> [CGPoint]
    {
        ...
        if arrValueYAxis.count > 0 {
            ...
            positions.reserveCapacity(arrValueYAxis.count)
            
            for valueY in arrValueYAxis {
                positions.append(CGPoint(x: 0.0, y: valueY.value))
            }

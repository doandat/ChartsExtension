...
open class LineChartRenderer: LineRadarRenderer
{
    ...
    @objc open func drawLinear(context: CGContext, dataSet: ILineChartDataSet)
    {
        ...
        for j in stride(from: _xBounds.min, through: _xBounds.range + _xBounds.min, by: 1)
        {
            ...
            if dataSet.entryCount > j + 1 {
                if let eNext: ChartDataEntry = dataSet.entryForIndex(j+1), eNext.data == nil {
                    continue
                }
            }
            if e == nil { continue }
            if e.data == nil {continue}
...
//            drawHighlightLines(context: context, point: pt, set: set)
            if let iconHighlight = e.iconHighlight {
                ChartUtils.drawImage(context: context,
                                     image: iconHighlight,
                                     x: pt.x + set.iconsOffset.x,
                                     y: pt.y + set.iconsOffset.y,
                                     size: iconHighlight.size)
            } else {
//                drawHighlightLines(context: context, point: pt, set: set)
